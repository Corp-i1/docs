...
C1Logger auto-initializes when you make your first log call. Simply set properties before logging:

**Benefits of Auto-Initialization:**
- **Simplified setup**: No need to remember to call `InitLog()`
- **Error reduction**: Prevents common initialization mistakes
- **Cleaner code**: Less boilerplate in your applications
- **Thread-safe**: Initialization is automatically thread-safe
...
The `C1LogConfig` class and `InitLog` methods are deprecated but still supported:
